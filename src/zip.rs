            ...
            let merge_listener = {
                ...
                async move {
                    ...
                    while let Some(task) = rx.recv().await {
                        match task {
                            FileTask::CompressData { file_index, data } => {
                    ...
                    for (index, (_file_name, director)) in sorted_dirs.into_iter().enumerate() {
                        ...
                        scope.spawn(async move {
                            ...
                            tx.send(FileTask::CompressData {
                                file_index: index,
                                data: local_header_writer.into_inner(),
                            })
                            .await
                            .map_err(|e| Error::Err(Box::new(e)))?;
...
enum FileTask {
    CompressData { file_index: usize, data: Vec<u8> },
